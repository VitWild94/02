        ...
        for event in self.longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                if event.text.lower() == 'привет':
                    ...
                    self.params = self.vk_tools.get_profile_info(event.user_id)
                    self.message_send(
                        event.user_id, f'Введите "да" для поиска в вашем городе. '
                                        f'Или введите название города?')
                elif event.text.lower() == 'да' or event.text.lower() == 'city':
                      self.message_send(
                        event.user_id, f'Введите возраст для поиска')
                elif event.text.lower() == f'возраст':                                 
                    '''Логика для поиска анкет'''
                    self.message_send(
                        event.user_id, 'Начинаю поиск')
                    if self.worksheets:
